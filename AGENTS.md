...
Own the pure, data-driven Eight Ball definitions and semantic build-trigger extraction. The catalog is the single source for the 32 item IDs, names, Bank/Combination/Direct Pot/Multi-Pot/Same-Pocket/Double Tap/Ball Tap families, unusual Tap effect metadata, add-Haul/add-Mult/xMult values, Legendary retrigger metadata, rarity, weights, and tooltip text. The trigger evaluator converts analyzed predicted or authoritative Shot Ledgers into ordered value-only Bank, Combination, Direct Pot, Multi-Pot, Same-Pocket, cue-recontact, and unique-object-tap occurrences using stable scoring-ball IDs and exact semantic event indices.
...
### `scripts/RogueliteBuildEffectEvaluator.gd`

Owns the pure, side-effect-free conversion from an analyzed Shot Ledger, ordered trigger occurrences, and immutable owned-item snapshot into modifier context, engine events, simulated state, and explicit authoritative mutations. It implements reusable numeric, persistent-scaler, cross-family, shot-ordinal, threshold-family-retrigger, and existing family-retrigger effect kinds with canonical phase ordering and one-generation retrigger bounds. Prediction simulates state without mutation; only completed authoritative settlement may apply returned mutations.

### `scripts/RogueliteBuildSystem.gd` / `scripts/RogueliteRewardSystem.gd`

Own The Long Sink's run-local five-slot nonphysical Eight Ball build and Mark Your Course economy. `RogueliteBuildSystem.gd` owns acquisition order, duplicate rejection, monotonic owned-item instance identity, value-only persistent item state, fixed-slot replacement, round persistence, fresh-run clearing, rewind snapshots, duplicate-safe authoritative mutation application, isolated Shot Lab loadouts, activation diagnostics, and predicted/authoritative parity observations. `RogueliteRewardSystem.gd` owns deterministic weighted three-offer generation, owned/contextual support exclusion, offer-family diversity, at-most-one-Legendary enforcement, full-tray replacement staging, support-loss warnings, and Keep Current Course. Mark Your Course offers Eight Balls only; obsolete utility-reward fields in older debug snapshots are ignored.
